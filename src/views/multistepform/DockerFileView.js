...
export const DockerFileView = (e) => {
  ...
  const inputdata = {
    ...
    imagename: "openmpi",
    imagetag: "4.1.0",
    mpi_configure_options: "--disable-fort",
    mpi_make_options: "-j4",
    mpi_type: "OpenMPI",
    openMPI_Major_Version: "v4.1",
    openMPI_Version: "v4.1",
  ...
  const dataForDockerfile = (data) => {
    ...
    if (data.mpi_type == "IntelMPI") {
      ...
      fdockerfilename = `DockerFile${inputdata.finalimagename}`;
      setDockerPushBuildCommand(
        `docker image build -t ${inputdata.imagename}:${inputdata.imagetag} --build-arg MPI_VERSION=${inputdata.intel_mpi_devel_version} --build-arg ICC_VERSION="${inputdata.intel_icc_version}" --build-arg MKL_VERSION=${inputdata.intel_mkl_version} --build-arg TBB_VERSION=${inputdata.intel_tbb_version}`
      );
      setBuildCommand(
        `docker image build -t ${inputdata.imagename}:${inputdata.imagetag} --build-arg MPI_VERSION=${inputdata.intel_mpi_devel_version} --build-arg ICC_VERSION="${inputdata.intel_icc_version}" --build-arg MKL_VERSION=${inputdata.intel_mkl_version} --build-arg TBB_VERSION=${inputdata.intel_tbb_version} . -f ${docfilename}`
      );
      setDockerBuildAppCommand(
        `docker image build -t ${inputdata.finalimagename}:${inputdata.finalimagetag} . -f ${fdockerfilename}`
      );
      setDockerFileName("DockerfileIntelMPI");
      setFinalDockerfilename(`DockerFile${inputdata.finalimagename}`);
      ...
      fdockerfilename = `DockerFile${inputdata.finalimagename}`;
      setDockerPushBuildCommand(
        `docker image build -t ${inputdata.imagename}:${inputdata.imagetag} --build-arg MPI_VERSION=${inputdata.mpich_Version} --build-arg MPI_CONFIGURE_OPTIONS="${inputdata.mpi_configure_options}" --build-arg USER=${inputdata.user} --build-arg WORKDIR=${inputdata.workdir}`
      );
      setBuildCommand(
        `docker image build -t ${inputdata.imagename}:${inputdata.imagetag} --build-arg MPI_VERSION=${inputdata.mpich_Version} --build-arg MPI_CONFIGURE_OPTIONS="${inputdata.mpi_configure_options}" --build-arg USER=${inputdata.user} --build-arg WORKDIR=${inputdata.workdir} . -f ${docfilename}`
      );
      setDockerBuildAppCommand(
        `docker image build -t ${inputdata.finalimagename}:${inputdata.finalimagetag} . -f ${fdockerfilename}`
      );
